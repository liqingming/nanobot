...
def outbound_message_for_event(
    *,
    ...
    if isinstance(event, ProgressEvent):
        meta.setdefault("_progress", True)
        if event.tool_hint:
            meta.setdefault("_tool_hint", True)
        if event.reasoning:
            meta.setdefault("_reasoning", True)
        if event.reasoning_delta:
            meta.setdefault("_reasoning_delta", True)
        if event.reasoning_end:
            meta.setdefault("_reasoning_end", True)
        if event.tool_events is not None:
            meta.setdefault("_tool_events", event.tool_events)
        if event.file_edit_events is not None:
            meta.setdefault("_file_edit_events", event.file_edit_events)
