...
class WriteFileTool(_FsTool):
    ...
    def description(self) -> str:
        return (
            ...
            "directories as needed. The result is a TEXT FILE saved on disk — "
            "not an image or picture. For code changes or partial edits, prefer "
            "apply_patch; use edit_file only for small exact replacements. "
            "After writing, you MUST end your response with the exact absolute "
            "path returned by this tool, e.g. '完整内容已保存至：<path>'."
        )
...
class EditFileTool(_FsTool):
    ...
    def summarize_result(self, args: dict[str, Any], result: Any) -> str:
        ...
        if result.startswith("Error"):
            return extract_error_summary(result)
        # EditFileTool replaces old_text with new_text; report delta.
        old = args.get("old_text", "") if isinstance(args, dict) else ""
        new = args.get("new_text", "") if isinstance(args, dict) else ""
