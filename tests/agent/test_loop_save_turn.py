...
def test_save_turn_strips_runtime_context_prefix_from_string() -> None:
    """Fork uses runtime-context PREFIX mode (prefix lines + user content at end),
    so _save_turn splits at the TAG and keeps the post-prefix user content."""
    loop = _mk_loop()
    session = Session(key="test:prefix-strip")
    runtime = (
