...
async def test_fixed_session_requests_are_serialized(aiohttp_client) -> None:
    ...
    assert r1.status == 200
    assert r2.status == 200
    # Requests are serialized: one must fully complete before the other starts.
    # The lock does not guarantee which runs first, so check either ordering.
    assert (
        order.index("end:first") < order.index("start:second")
        or order.index("end:second") < order.index("start:first")
    )
...
async def test_json_request_passes_workspace_and_timeout_to_agent(aiohttp_client, tmp_path) -> None:
    ...
    resp = await client.post(
        ...
        json={
            ...
            "timeout": 30,
            "tool_policy": {"blocked_grep_paths": ["Assets"], "blocked_read_file_paths": ["Assets/ResourcesAssets"], "allowed_grep_paths": ["Assets/Script/Game/moduls/DragonInvadeActivity"], "disable_exec": True},
        },
    )
    ...
    assert kwargs["metadata"]["tool_policy"] == {"blocked_grep_paths": ["Assets"], "blocked_read_file_paths": ["Assets/ResourcesAssets"], "allowed_grep_paths": ["Assets/Script/Game/moduls/DragonInvadeActivity"], "disable_exec": True}
